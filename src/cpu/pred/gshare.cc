...
// GShare Path Predictor - Implementation File
...
// Constructor for the GShareBP branch predictor
GShareBP::GShareBP(const GShareBPParams &params)
  : BPredUnit(params), // Always need to pass params object to superclass
    ...
    // Rest of the below member variables are initialized specific to GShareBP based on param values
...
bool GShareBP::lookup(ThreadID tid, Addr pc, void * &bp_history)
{
  ...
  // xor'd with the current branch address
  unsigned predIdx = (globalHistoryReg[tid] ^ (pc >> instShiftAmt)) & historyRegisterMask;
  ...
  // Create a new GShare history object containing the current history and prediction made
  GShareHistory* hist = new GShareHistory;
...
void GShareBP::updateHistories(ThreadID tid, Addr pc, bool uncond, bool taken,
...
// inst: UNUSED
// target: The target address of the branch instruction. Currently UNUSED.
void GShareBP::update(ThreadID tid, Addr pc, bool taken,
                   ...
                   const StaticInstPtr & inst, Addr target)
{
  ...
  unsigned predIdx = (history->globalHistoryReg ^ (pc >> instShiftAmt)) & historyRegisterMask;
